...
from family_spending.settings import CATEGORIES_FILE, MERCHANTS_FILE
...
def _require_non_empty_string(value: object, label: str, path: Path) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise MappingDataError(
            f"Invalid {label} in {path}: expected a non-empty string, got {value!r}"
        )
    return value
...
def load_merchant_mappings(
    ...
    """Load the reviewed Merchant and default-Category rules used to initialize missing Enrichment state."""

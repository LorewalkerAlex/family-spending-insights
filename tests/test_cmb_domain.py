...
from family_spending.transaction_resolution import build_cmb_domain_state, build_household_domain_state, resolve_transactions
...
def _cmb(
    ...
    *,
    ...
    """Keep fixtures explicit about source identity because the domain separates it from system identity."""
...
class SourceAndReconciliationTests(unittest.TestCase):
    def test_cmb_adapter_preserves_source_fields_without_reusing_source_id_as_transaction_id(self) -> None:
        """The Source boundary must be lossless while keeping source and system identity visibly different."""
    ...
    def test_cmb_reconciliation_reuses_identity_and_refreshes_authoritative_facts(self) -> None:
        """Reruns must be idempotent while changed authoritative CMB facts refresh the same Transaction identity."""
    ...
    def test_cmb_reconciliation_upgrades_existing_supporting_link_to_authoritative(self) -> None:
        """When CMB arrives for a supporting link, its authority must be explicit rather than inherited accidentally."""
        ...
        self.assertEqual(
            rerun.source_links,
            (TransactionSourceLink(transaction_id, record.id, "authoritative"),),
        )


class EnrichmentTests(unittest.TestCase):
    def test_persisted_transaction_override_survives_domain_rebuild(self) -> None:
        """Historical transaction exceptions now live only in authoritative Enrichment state and survive rebuilds unchanged."""
        raw = (_cmb("cmb_exception", "2026-01-02", "3660.81", "京东支付-电视", source_index=1),)
        mappings = _mappings(
            descriptions={"京东支付-电视": "京东购物"},
            categories={"京东购物": "综合购物", "家居商户": "家居家电"},
        )
        initial = build_cmb_domain_state(raw, mappings)
        transaction = initial.reconciliation.transactions[0]
        persisted = TransactionEnrichmentState(
        ...
        rebuilt = build_household_domain_state(
            raw,
            (),
            mappings,
            existing_links=initial.reconciliation.source_links,
            existing_enrichment_states={transaction.id: persisted},
        )
        enrichment = rebuilt.enrichments_by_transaction_id[transaction.id]
    ...
    def test_other_expense_review_remains_enrichment_based(self) -> None:
        """Category-only review signals remain stable even though high-value review runs after refund netting."""
...
class RefundAndStatisticsTests(unittest.TestCase):
    def test_refund_netting_preserves_core_transactions_and_returns_positive_net_consumption(self) -> None:
        """Refund analysis derives net spending without mutating authoritative purchase and refund facts."""
        raw = (
    ...
    def test_same_merchant_fallback_can_match_different_descriptions_within_30_days(self) -> None:
        """Merchant identity stays bounded evidence so current reviewed refund behavior survives the model split."""
        raw = (
    ...
    def test_statistics_join_transaction_and_current_enrichment_without_materialized_enriched_transaction(self) -> None:
        """Analytics joins amounts, counts, category, and display name from separate current domain states."""
        raw = (

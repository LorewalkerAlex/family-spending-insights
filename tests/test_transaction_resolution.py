...
class TransactionResolutionTests(unittest.TestCase):
    ...
    def test_batch_preserves_order_and_groups_results(self) -> None:
        transactions = (
        ...
        self.assertEqual(
            tuple(item.transaction.transaction_id for item in batch.transactions),
        ...
        self.assertEqual(
            tuple(item.transaction.transaction_id for item in batch.unclassified),
            ("cmb_unclassified",),
        )
        self.assertEqual(
            tuple(
        ...
        self.assertEqual(
            tuple(
                item.transaction.transaction_id
                for item in batch.reviews_by_signal[HIGH_VALUE_GENERAL_SHOPPING_REVIEW]
            ),
            ("cmb_high_value",),
        )
    ...
    def test_report_includes_summary_and_only_actionable_details(self) -> None:
        transactions = (
        ...
        self.assertIn("cmb_unclassified", report)
        self.assertIn("cmb_other", report)
        self.assertNotIn("transaction_id=cmb_override", report)
    ...
    def test_main_converts_known_errors_to_clean_cli_failure(self) -> None:
        ...
        with patch(
            ...
            with self.assertRaisesRegex(SystemExit, "Transaction resolution failed.*inconsistent official data"):
                main()

...
def make_transaction(
    ...
    *,
    ...
    """Use raw positive spending because the diagnostic entrypoint owns refund-aware review evaluation."""
...
def write_mapping_files(root: Path) -> tuple[Path, Path]:
    """Keep file-based tests on the two reviewed Mapping inputs after retiring the historical transaction-specific Mapping input."""
...
class TransactionResolutionTests(unittest.TestCase):
    def setUp(self) -> None:
        """Load official-style Mapping files once so diagnostics exercise the normal default-only bootstrap path."""
    ...
    def test_batch_preserves_source_order_and_groups_results(self) -> None:
        """System IDs may differ from source IDs while classification and review grouping remain deterministic."""
        transactions = (
    ...
    def test_file_resolution_reads_only_transactions_and_mapping_defaults(self) -> None:
        """The diagnostic path must no longer require a transaction-specific Mapping input file."""
    ...
    def test_report_exposes_system_and_source_identity_only_for_actionable_details(self) -> None:
        """Reviewers need both IDs for actionable rows while ordinary mapped rows should not clutter the report."""
        transactions = (
            make_transaction("cmb_appliance", "支付宝-测试家电", source_index=1),
        ...
        self.assertIn("Transaction overrides: 0", report)
        ...
        self.assertNotIn("source_record_id=cmb_appliance", report)
